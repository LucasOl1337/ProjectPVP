...
const SkillRuntime = preload("res://engine/scripts/characters/skills/skill_runtime.gd")
...
func configure(character_data: CharacterData, owner: Node) -> void:
	...
	var skills: Array = character_data.skills
	...
	if slot_names.size() == skills.size() and slot_names.size() > 0:
		...
		for i in range(slot_names.size()):
			...
			var skill: Resource = skills[i] as Resource
