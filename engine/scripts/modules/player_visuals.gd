...
func _is_headless_runtime() -> bool:
	...
	if OS.has_method("get_cmdline_user_args"):
		...
		return args.has("--no-visuals")

	return false



const CharacterBaseProfile = preload("res://engine/scripts/characters/character_base_profile.gd")

const SpriteDebugOverlay = preload("res://engine/scripts/modules/sprite_debug_overlay.gd")
...
const AIM_ARROW_TEXTURE_PATH := "res://visuals/assets/ui/aim_indicator/aim_arrow.png"

const AIM_SEGMENT_TEXTURE_PATH := "res://visuals/assets/ui/aim_indicator/aim_segment.png"
...
const TEAM_RECOLOR_SHADER = preload("res://visuals/assets/shaders/team_recolor.gdshader")
...
func configure_character_visuals() -> void:
	...
	var use_sprite := _sprite_has_frames() or _has_cached_character_assets()
...
func setup_character_sprite() -> void:
	...
	configure_character_visuals()



func _sprite_has_frames() -> bool:
...
func setup_aim_indicator() -> void:
	...
	var arrow_texture: Variant = null
	if ResourceLoader.exists(AIM_ARROW_TEXTURE_PATH):
		arrow_texture = load(AIM_ARROW_TEXTURE_PATH)
	...
	var segment_texture: Variant = null
	if ResourceLoader.exists(AIM_SEGMENT_TEXTURE_PATH):
		segment_texture = load(AIM_SEGMENT_TEXTURE_PATH)
...
func update_character_sprite(aim_input: Vector2, dash_active: bool, facing: int, is_dead: bool, on_floor: bool, velocity_x: float, aim_hold_active: bool) -> void:
	...
	if character_sprite == null or not _sprite_has_frames():
...
func _has_character_assets(base_path: String) -> bool:
	...
	var dir := DirAccess.open(rotations_path)

	if dir == null and rotations_path.begins_with("res://"):

		dir = DirAccess.open(ProjectSettings.globalize_path(rotations_path))

	if dir == null:

		var anim_path := base_path + "animations/"

		var anim_dir := DirAccess.open(anim_path)

		if anim_dir == null and anim_path.begins_with("res://"):

			anim_dir = DirAccess.open(ProjectSettings.globalize_path(anim_path))

		return anim_dir != null
...
func _has_cached_character_assets() -> bool:
	...
	if _sprite_has_frames():

		_cached_assets_base_path = _character_base_path()

		_cached_has_assets = true
...
func _set_crouch_visuals(active: bool) -> void:
	...
	var has_character_sprite := character_sprite != null and _sprite_has_frames()
...
func _apply_collider_override(action_key: String) -> void:
	...
	if character_data != null and character_data.action_collider_overrides.has(action_key):
		...
		if override_value is Dictionary:
			...
			var override_dict: Dictionary = override_value as Dictionary
...
func _load_directional_rotations(frames: SpriteFrames, rotations_path: String) -> void:
	...
	for dir_name in direction_files.keys():
		...
		for file_name in direction_files[dir_name]:
			...
			var texture := _try_load_frame_texture(full_path)
...
func _load_animation_set(frames: SpriteFrames, base_path: String, action: String, speed: float) -> bool:
	...
	if _has_direction_subfolders(base_path):
		...
		for dir_name in DIRECTION_FOLDER_ALIASES.keys():
			...
			var resolved_dir := _find_existing_direction_folder(base_path, DIRECTION_FOLDER_ALIASES[dir_name])
...
func _ensure_action_fallback(frames: SpriteFrames, action: String) -> void:

	if frames.has_animation(action) and frames.get_frame_count(action) > 0:

		return

	var source := ""

	var down := "%s_down" % action

	if frames.has_animation(down) and frames.get_frame_count(down) > 0:
		...
		for dir_name in DIRECTION_PRIORITY:
			...
			var candidate := "%s_%s" % [action, dir_name]
	...
	if source != "":
		...
		_copy_animation(frames, source, action)





func _ensure_direction_fallbacks(frames: SpriteFrames, action: String) -> void:
	...
	for dir_name in DIRECTION_FOLDER_ALIASES.keys():
		...
		var target := "%s_%s" % [action, dir_name]

		if frames.has_animation(target) and frames.get_frame_count(target) > 0:

			continue

		_copy_animation(frames, action, target)
...
const DIRECTION_FOLDER_ALIASES := {
...
const DIRECTION_PRIORITY := ["down", "right", "left", "up", "down_right", "down_left", "up_right", "up_left"]





func _try_load_frame_texture(resource_path: String) -> Texture2D:

	var tex: Texture2D = load(resource_path)
...
func _load_frames_from_folder(frames: SpriteFrames, animation_name: String, folder_path: String) -> bool:
	...
	for file_name in files:
		...
		var texture := _try_load_frame_texture(folder_path + file_name)
	...
	if added <= 0:
		...
		print("[ANIM] WARNING: found %d files but loaded 0 frames for '%s' from '%s'" % [files.size(), animation_name, folder_path])
...
func _load_frames_from_pattern(frames: SpriteFrames, animation_name: String, base_path: String, dir_name: String) -> bool:
	...
	for file in files:
		...
		var texture := _try_load_frame_texture(base_path + file)
	...
	if added <= 0:
		...
		print("[ANIM] WARNING: found %d files but loaded 0 frames for '%s' from '%s'" % [files.size(), animation_name, base_path])
