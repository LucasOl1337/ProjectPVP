...
- Runner headless do Godot: `res://tools/headless_training_runner.gd`
- Orquestrador: `tools/island_orchestrator.py`
- Config: `IA/config/islands.json`

## Como rodar (menu)

1) Ajuste o caminho do Godot em `IA/config/islands.json` (campo `godot_exe`).
2) Rode:

```bash
python tools/island_orchestrator.py menu --config IA/config/islands.json
```
...
```bash
python tools/island_orchestrator.py run --config IA/config/islands.json
```

Dry-run:

```bash
python tools/island_orchestrator.py run --config IA/config/islands.json --dry-run
```

## Saídas / Feedback

- Cada worker salva em `IA/weights/islands/round_XXXX/worker_N/`:
  - `best.json` (genoma vencedor daquele worker)
  - `result.json` (fitness final, caminhos e parâmetros)
  - `genetic_log.csv` (log por geração)
- Os `topk` da rodada ficam em `IA/weights/islands/round_XXXX/top/`.
