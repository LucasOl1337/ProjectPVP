...
protocol:
  ...
  version: 1
canonical_contracts:
  observation_builder: "scripts/modules/bot_observation_builder.gd"
  action_schema: "scripts/modules/bot_action_frame.gd"
  python_trainer: "tools/training_genetic_ga.py"
```

Este documento descreve **como a IA funciona hoje** no Project PVP (bots in-game + treino externo via Python), quais são os **contratos de dados** (observação/ação/bridge), e como **continuar evoluindo** sem quebrar o jogo, o treino ou os artefatos em `IA/`.
...
O diretório `IA/` é o **hub de configuração e artefatos**: configs do treino, bots (recompensas por jogador), pesos do genoma e logs.
...
### `IA/config/trainer.json` (auto-start do Python pelo Godot)
...
`IA/weights/best_genome.json` é lido pela policy `genetic` no Godot (ver [bot_policy_genetic.gd](file:///c:/Users/user/CascadeProjects/Project%20PVP/scripts/modules/bot_policy_genetic.gd)).
...
Observação: em builds/export, `res://` tende a ser read-only. O `TrainingManager` tenta abrir `res://IA/logs/...` e pode cair para `user://...` se não conseguir.
...
- **Configurações**: se adicionar/remover keys em `IA/config/*.json`.
