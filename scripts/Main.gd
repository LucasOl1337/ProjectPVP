...
const InputMapConfig = preload("res://scripts/modules/input_map_config.gd")
const ArenaManager = preload("res://scripts/modules/arena_manager.gd")
...
const NetMatchDriver = preload("res://scripts/modules/net_match_driver.gd")
const BotDriver = preload("res://scripts/modules/bot_driver.gd")
const TrainingManagerScript = preload("res://scripts/modules/training_manager.gd")
const SuddenDeathController = preload("res://scripts/modules/sudden_death_controller.gd")
const MAX_WINS := 5
const MAX_STATE_FRAMES := 240
const IA_CONFIG_DIR := "res://IA/config"
...
func _get_player_half_width(player: Node) -> float:
	...
	if shape_node == null or not (shape_node is CollisionShape2D):
		return 0.0
	var collider := shape_node as CollisionShape2D
...
func _start_live_training() -> void:
	...
	_start_trainer_process("python", "tools/training_genetic_ga.py", args, true)
...
func _load_trainer_config() -> Dictionary:
	var defaults := {
		"python_path": "python",
		"script_path": "tools/training_genetic_ga.py",
...
func _sync_trainer_ui() -> void:
	...
	var script_path := String(trainer_config.get("script_path", "tools/training_genetic_ga.py"))
...
func _set_trainer_status(text: String) -> void:
	...
	if dev_hud and dev_mode_enabled:
		dev_hud.configure_trainer(
			...
			String(trainer_config.get("script_path", "tools/training_genetic_ga.py")),
...
func _append_default_trainer_args(args: PackedStringArray) -> PackedStringArray:
	...
	if not _args_contain_flag(result, "--load-path"):
		var default_seed := "res://BOTS/%s/best_genome.json" % DEFAULT_PROFILE
...
func _configure_collision_masks() -> void:
	...
	mask |= CollisionLayers.PLAYER_BODY
	if player_one:
