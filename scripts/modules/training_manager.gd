...
const RewardShaperDefault = preload("res://scripts/modules/reward_shaper_default.gd")
const RewardShaperTunable = preload("res://scripts/modules/reward_shaper_tunable.gd")
const TrainingRecorder = preload("res://scripts/modules/training_recorder.gd")

const BASE_TICK_RATE := 60.0

const TrainingBridge = preload("res://scripts/modules/training_bridge.gd")
...
var round_log_path := "res://IA/logs/round_stats.csv"
var evolution_stats: Dictionary = {}
var evolution_log_path := "res://IA/logs/evolution.csv"
...
func _record_step(step_payload: Dictionary) -> void:
	...
	if not (obs_value is Dictionary):
		return
	var obs_map := obs_value as Dictionary
...
func _update_evolution_stats() -> void:
	...
	for round_entry in round_history:
		if not (round_entry is Dictionary):
			continue
		var round_dict := round_entry as Dictionary
...
func _determine_match_winner() -> int:
	...
	if not (wins_value is Dictionary):
		return 0
	var wins := wins_value as Dictionary
