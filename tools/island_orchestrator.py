...
def build_trainer_cmd(
    ...
    cmd = [
        python_exe,
        str(project_root / "tools" / "training_genetic_ga.py"),
        "--host",
        "127.0.0.1",
        "--port",
        str(int(port)),
...
def default_config(project_root: Path) -> Dict:
    return {
        ...
        "match_rules_path": "IA/config/match_rules.json",
        "state_dir": "IA/weights/islands",
        "initial_seed": "IA/weights/best_genome.json",
        "promote_best": True,
        "promote_best_path": "IA/weights/best_genome.json",
        "promote_state_path": "IA/weights/current_bot.json",
...
def promote_best_genome(project_root: Path, cfg: Dict, summary: Dict) -> None:
    ...
    if not src.exists():
        return
    promote_path = Path(resolve_path(project_root, str(cfg.get("promote_best_path", "IA/weights/best_genome.json"))))
    state_path = Path(resolve_path(project_root, str(cfg.get("promote_state_path", "IA/weights/current_bot.json"))))
...
def main() -> int:
    ...
    parser.add_argument("--config", default="IA/config/islands.json")
